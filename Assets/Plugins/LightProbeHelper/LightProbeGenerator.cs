...
public class LightProbeGenerator : MonoBehaviour
{
    ...
    List<Vector3> GetProbesForVolume_Random(Bounds ProbeVolume, int Count)
    {
        ...
        {
            Vector3 probePos = ProbeVolume.center + new Vector3(Random.Range(-0.5f, 0.5f) * ProbeVolume.extents.x, Random.Range(-0.5f, 0.5f) * ProbeVolume.extents.y, Random.Range(-0.5f,
                                                                0.5f) * ProbeVolume.extents.z);
            probePositions.Add(probePos);
        }
